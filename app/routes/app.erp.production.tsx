...
async function resolveShopifyImageForProductionItem(admin: any, item: any) {
  let productGid = snapshotValue(item, "shopifyProductGid") || snapshotValue(item, "shopifyProductId") || item.shopifyProductGid || null;
  let variantGid = snapshotValue(item, "shopifyVariantGid") || snapshotValue(item, "shopifyVariantId") || item.shopifyVariantGid || null;
  let productImageUrl = firstImageFromQuoteItem(item) || item.productImageUrl || "";

  if (admin && variantGid && !productImageUrl) {
    ...
    if (variant) {
      variantGid = variant.id || variantGid;
      productGid = variant.product?.id || productGid;
      productImageUrl = variant.image?.url || variant.product?.featuredImage?.url || variant.product?.images?.nodes?.[0]?.url || productImageUrl;
    }
  }

  if (admin && productGid && !productImageUrl) {
    const response = await admin.graphql(
      `#graphql
    ...
    if (product) {
      productGid = product.id || productGid;
      productImageUrl = product.featuredImage?.url || product.images?.nodes?.[0]?.url || productImageUrl;
    }
  }

  if (admin && !productImageUrl && (item.sku || item.productName || item.productTitle)) {
    const queryValue = item.sku ? `sku:${item.sku}` : `title:*${item.productName || item.productTitle}*`;
    const response = await admin.graphql(
      `#graphql
        query FindProductImage($query: String!) {
    ...
    if (product) {
      const variant = item.sku ? product.variants?.nodes?.find((node: any) => String(node.sku || "").toLowerCase() === String(item.sku || "").toLowerCase()) : product.variants?.nodes?.[0];
      productGid = product.id || productGid;
      variantGid = variant?.id || variantGid;
      productImageUrl = variant?.image?.url || product.featuredImage?.url || product.images?.nodes?.[0]?.url || productImageUrl;
    }
  }

  return { productImageUrl, shopifyProductGid: productGid, shopifyVariantGid: variantGid };
}

async function createProductionJobFromQuote(shop: string, quoteId: string, admin?: any) {
  ...
  const hydratedItems = await Promise.all(
    quote.items.map(async (item: any) => ({
      item,
      imageData: await resolveShopifyImageForProductionItem(admin, item),
    })),
  );
  ...
  const job = await db.productionJob.create({
    data: {
      ...
      productImageUrl: hydratedItems[0]?.imageData?.productImageUrl || firstImageFromQuoteItem(quote.items[0]),
      proofUrl: null,
      items: {
        create: hydratedItems.map(({ item, imageData }: any, index: number) => ({
          shop,
          quoteItemId: item.id,
          ...
          productImageUrl: imageData.productImageUrl || firstImageFromQuoteItem(item) || null,
          shopifyProductGid: imageData.shopifyProductGid || snapshotValue(item, "shopifyProductGid") || snapshotValue(item, "shopifyProductId") || null,
          shopifyVariantGid: imageData.shopifyVariantGid || snapshotValue(item, "shopifyVariantGid") || snapshotValue(item, "shopifyVariantId") || null,
  ...
  const syncedImages = hydratedItems.filter((entry: any) => entry.imageData?.productImageUrl).length;
  if (syncedImages) {
    await createEvent(shop, job.id, "shopify_images_synced", `${syncedImages} Shopify product image(s) attached automatically.`);
  }
...
export async function action({ request }: { request: Request }) {
  ...
  if (intent === "createFromQuote") {
    ...
    const result = await createProductionJobFromQuote(shop, quoteId, admin);
  ...
  if (intent === "syncShopifyImages") {
    const jobId = String(formData.get("jobId") || "");
    const job = await db.productionJob.findFirst({ where: { shop, id: jobId }, include: { items: true } });
    if (!job) return Response.json({ ok: false, message: "Job not found." }, { status: 404 });

    let syncedCount = 0;
    let mainImageUrl = job.productImageUrl || "";

    for (const item of job.items || []) {
      const imageData = await resolveShopifyImageForProductionItem(admin, item);
      if (!imageData.productImageUrl) continue;
      syncedCount += 1;
      if (!mainImageUrl) mainImageUrl = imageData.productImageUrl;
      await db.productionJobItem.update({
        where: { id: item.id },
        data: {
          productImageUrl: imageData.productImageUrl,
          shopifyProductGid: imageData.shopifyProductGid || item.shopifyProductGid,
          shopifyVariantGid: imageData.shopifyVariantGid || item.shopifyVariantGid,
        },
      });
    }

    if (mainImageUrl) {
      await db.productionJob.update({ where: { id: job.id }, data: { productImageUrl: mainImageUrl } });
      const existingImageFile = await db.productionJobFile.findFirst({ where: { shop, jobId, fileUrl: mainImageUrl, fileType: "image" } });
      if (!existingImageFile) {
        await db.productionJobFile.create({
          data: {
            shop,
            jobId,
            fileName: "Shopify product image",
            fileType: "image",
            fileUrl: mainImageUrl,
            notes: "Auto-synced from Shopify product/variant image.",
          },
        });
      }
    }

    await createEvent(shop, jobId, "shopify_images_synced", syncedCount ? `${syncedCount} Shopify image(s) synced.` : "No Shopify images found for this job yet.");
    return Response.json({ ok: true, message: syncedCount ? `Synced ${syncedCount} Shopify image(s).` : "No Shopify images found." });
  }
      ...
      <BlockStack gap="400">
        <InlineStack align="space-between" blockAlign="start">
          ...
          <InlineStack gap="200">
            ...
            <Form method="post">
              <input type="hidden" name="intent" value="syncShopifyImages" />
              <input type="hidden" name="jobId" value={job.id} />
              <Button submit>Sync Shopify images</Button>
            </Form>
