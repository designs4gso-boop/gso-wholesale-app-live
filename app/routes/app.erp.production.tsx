...
function buildJobTicket(quote: any) {
  const firstItem = quote.items?.[0] || {};
  const qty = firstItem.quantity || quote.items?.reduce((sum: number, item: any) => sum + Number(item.quantity || 0), 0) || 1;
  const itemKey = slugPart(firstItem.sku || firstItem.productName || "JOB");
  const shortQuote = slugPart(String(quote.id || "QUOTE").slice(-6), "QUOTE");
  return `GSO-${dateStamp()}-${shortQuote}-${itemKey}-${qty}`;
}
...
async function createProductionJobFromQuote(shop: string, quoteId: string) {
  ...
  const jobTicket = buildJobTicket(quote);
