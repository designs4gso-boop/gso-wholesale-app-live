...
- **Machine routing v1 (conservative — an open owner decision):** CMYK-only work routes to the confirmed Mimaki hot folder `\\DESKTOP-KD7NQTS\GSO_MIMAKI_CMYK_STANDARD` (explicit Mimaki assignment or the documented no-tag default). **White/gloss jobs are review-only**: the 13A.6G request said "Mimaki for white/gloss" but the repo's documented rule (13A.4), finish-presets (`preferredMachine: Roland LG-540` for white/gloss), the old intake config (white/gloss -> VersaWorks LG-640 folders), and the confirmed folder's own name (CMYK_STANDARD) all say Mimaki is CMYK-only — contradictions go to review, never guessed. Roland/VersaWorks routing is configurable but DISABLED until its hot folder is confirmed. Both routing flags default OFF in the example config; nothing routes until the owner flips them after a dry run.
...
- **Before live routing:** owner must (1) run -Health and -DryRun on the shop PC, (2) confirm the white/gloss machine decision and the VersaWorks hot-folder path, (3) flip `MimakiRoutingEnabled` after the dry run, (4) install the scheduled task. Stop conditions honored: nothing committed/pushed/deployed, no task installed, routing flags ship OFF.
