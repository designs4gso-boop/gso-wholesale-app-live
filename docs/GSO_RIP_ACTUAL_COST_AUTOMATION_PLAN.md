...
- **13A.6B — Windows watcher + matching hardening** (next): `tools/gso-rasterlink-sync.ps1` polling `rasterlink\incoming` -> POST -> processed/error folders; re-match tooling (ripJobName↔sourceJobName secondary key), unmatched/ambiguous review list, backfill of `productionJobId`/`productionJobItemId`.
