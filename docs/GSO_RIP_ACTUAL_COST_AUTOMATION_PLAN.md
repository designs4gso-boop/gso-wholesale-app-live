...
- **13A.5 — Read-only Actual Cost Dashboard** (no schema, no writes): per matched PrintLogEntry compute actual ink $ (verified $/ml × channel ml, machine attributed via machineName → route token → printerSoftware fallback), media $ (mediaName name-match to Materials, unmatched listed for review), machine time $ (printMinutes × rate — display both $5 and $8 until owner picks), rolled up per job ticket. Buckets available NOW with current data.
- **13A.6 — Matching hardening** (existing columns, gated writes): re-match button (ticket, then ripJobName↔sourceJobName), unmatched-rows review list, backfill `productionJobId`/`productionJobItemId`.
