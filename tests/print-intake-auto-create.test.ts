...
describe("route-plan + creator source pins (15F.0J.5)", () => {
  ...
  it("agent v1.5 sends the full hash+size on the PLAN call; ledger dedupe unchanged; test 22 root-only scan intact", () => {
    expect(agentSrc).toContain("gso-print-intake-agent/1.5");
